...
extern
const char * server_port; //Listening port of the server
...
void client_operations::add_connection_list(int file_desc,
  const char * port) {
...
void client_operations::send_download_command(int file_desc,
  const char * send_cmd) {
...
bool client_operations::is_connection_present(const char * host,
  const char * port) {
...
int client_operations::connect_to_port(const char * host,
  const char * port) {
  int sockfd, numbytes;
  char hostname[MAXMSGSIZE];
  char service[20];
...
void client_operations::send_file_over_socket(int clientfd,
  const char * filename) {
