...
class client_operations: public server_operations {
  ...
  private: vector < host_info > connected_list;
  ...
  bool is_connection_present(const char * host,
    const char * port);
  ...
  void send_file_over_socket(int clientfd,
    const char * filename);
